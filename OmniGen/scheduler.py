...
model_name = "/home/zhouchao/data/OmniGen-v1"
...
class OmniGenScheduler:
    ...
    def __call__(self, z, func, model_kwargs, use_kv_cache: bool=True, offload_kv_cache: bool=True):
        ...
        from .utils import find_element_indices,intermediate_vars
        import os
        # import pickle
        # intermidiate_output_path = os.getenv("INTERMIDIATE_OUTPUT_PATH", "./junk/default_kv")
        # os.makedirs(intermidiate_output_path, exist_ok=True)
        # with open(f"{intermidiate_output_path}/input_ids.pkl", "wb") as f:
        #     pickle.dump(model_kwargs['input_ids'], f)
        # with open(f"{intermidiate_output_path}/input_image_sizes.pkl", "wb") as f:
        #     pickle.dump(model_kwargs['input_image_sizes'], f)
        # with open(f"{intermidiate_output_path}/z.pkl", "wb") as f:
        #     pickle.dump(z, f)
        
        conds = model_kwargs['input_image_sizes'][0]
        text = [text_tokenizer.decode(model_kwargs['input_ids'][0][i], skip_special_tokens=False) for i in range(conds[0][1]+2,len(model_kwargs['input_ids'][0])-5)]
        spilt_idx = [0] + find_element_indices(text,',') + [len(text)]
        spilt_idx = [i+conds[0][1]+2 for i in spilt_idx]
        intermediate_vars['spilt_idx'] = spilt_idx
        intermediate_vars['conds'] = conds
        intermediate_vars['res'] = conds[0][1]-conds[0][0]
        for i in tqdm(range(self.num_steps)):
            os.environ["CUR_STEP"] = str(i)
